...
function generateClimbs() {
  ...
  while(climbSet.size < 21) {
    climbSet.add( words.climbName[ randomInt(words.climbName.length) ] )
  }
  climbSet = [...climbSet]

  const active = climbSet.shift()
...
function injectData() {
  ...
  document.getElementById('active-climb-1').textContent = climbs.active
  document.getElementById('active-climb-2').textContent = climbs.active
  document.getElementById('active-climb-3').textContent = climbs.active
  for (const [index, climb] of climbs.others.entries()) {
    document.getElementById('random-climb-'+(index+1)).textContent = climb
  }
}

document.addEventListener("DOMContentLoaded", function() {
  injectData()
  document.getElementById('descriptor-gen').onclick = injectData;
})
